...
classes = ["Inertinite", "Vitrinite","high","low", "connect", ]
...
def get_img_name(img_dir, format="tif"):
    """
    获取文件夹下format格式的文件名
    :param img_dir: str
    :param format: str
...
def get_model(m_path, vis_model=False):
    ...
    resnet34.fc = nn.Linear(num_ftrs, 4)

    checkpoint = torch.load(m_path)
    # resnet34.load_state_dict(checkpoint['model_state_dict'])
    resnet34.load_state_dict(checkpoint)
    if vis_model:
        from torchsummary import summary
        summary(resnet34, input_size=(3, 224, 224), device="cpu")
...
if __name__ == "__main__":
    ...
    # 2. model
    resnet34 = get_model(model_path, True)
    resnet34.to(device)
    resnet34.eval()
