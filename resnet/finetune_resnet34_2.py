...
set_seed(1)  # 设置随机种子
label_name = {"Inertinite": 0, "Vitrinite": 1,"high": 2,"low":3 }
...
# ============================ step 1/5 数据 ============================
data_dir = os.path.abspath(os.path.join(BASEDIR, "..", "..", "data", "coal_data_split"))
...
flag = 1
if flag:
    path_pretrained_model = os.path.join(BASEDIR, "..", "..", "data", "resnet34-333f7ec4.pth")
...
flag = 0
if flag:
    fc_params_id = list(map(id, resnet34_ft.fc.parameters()))     # 返回的是parameters的 内存地址
    ...
    optimizer = optim.SGD(resnet34_ft.parameters(), lr=LR, momentum=0.9)               # 选择优化器

# scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=lr_decay_step, gamma=0.1)     # 设置学习率下降策略
scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=150, eta_min=0)     # 设置学习率下降策略
...
for epoch in range(start_epoch + 1, MAX_EPOCH):
    ...
    if (epoch+1) % val_interval == 0:
        ...
        with torch.no_grad():
            ...
            if valid_curve[-1]<=min(valid_curve):
                path_statedict_model = "./resnet34_model_2.pkl"
                models_w = torch.save(resnet34_ft.state_dict(), path_statedict_model)
        resnet34_ft.train()
